use rocket::{http::{ContentType, Status}, local::blocking::{Client, LocalResponse}, routes};
...
fn updating_users_works() {
    execute_with(DB_PATH, || {
        ...
        let rocket = rocket::build().manage(conn).mount("/", routes![register_user, user, update_user]);
        ...
        assert_eq!(parse_ok_response(response), User {
            id: 0,
            email: None,
            tg_handle: Some("@dummy".to_string()),
            notifier: Notifier::Telegram,
